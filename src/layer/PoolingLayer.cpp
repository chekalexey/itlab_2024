 ...
 public:
  ...
  void configure(TensorShape& input_shape, TensorShape& output_shape,
    ...
    try {
      ...
      if (!NEPoolingLayer::validate(
              input.info(), output.info(),
              PoolingLayerInfo(PoolingType::MAX, DataLayout::NHWC))) {
      ...
      pool_.configure(&input, &output,
                      PoolingLayerInfo(PoolingType::MAX, DataLayout::NHWC));

      configured_ = true;
    } catch (const std::exception& e) {
      configured_ = false;
      std::cerr << "PoolingLayer configuration error: " << e.what()
                << std::endl;
    }
  }
  ...
  void exec() override {
    if (!configured_) {
      throw std::runtime_error(
          "PoolingLayer: Layer not configured before exec.");
    }
    pool_.run();
  }
