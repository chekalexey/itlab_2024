...
class MatMulLayer : public Layer {
 ...
 public:
  ...
  void configure(TensorShape& input_x_shape, TensorShape& input_y_shape,
                 TensorShape& output_shape, Tensor& input_x, Tensor& input_y,
                 Tensor& output) {
    try {
      ...
      if (!NEMatMul::validate(input_x.info(), input_y.info(), output.info(),
                              MatMulInfo(), CpuMatMulSettings(),
                              ActivationLayerInfo())) {
      ...
      m_.configure(&input_x, &input_y, &output, MatMulInfo(),
                   CpuMatMulSettings(), ActivationLayerInfo());
      configured_ = true;
  ...
  void exec() override {
    if (!configured_) {
      throw std::runtime_error(
          "MatMulLayer: Layer not configured before exec.");
    }
    m_.run();
  }
